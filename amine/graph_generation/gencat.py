#
# source copied from the project GenCAT https://github.com/seijimaekawa/GenCAT
#
...
# modification cpasquie for graph with active modules
def gencat_active(n,m,k,d,max_deg,M,D,H,phi_c=1,omega=0.2,r=50,step=100,att_type="normal",woAP=False,woITS=False, nb_active=0, size_active=[]):
    # node degree generation 
    theta = node_deg(n,m,max_deg)
#     line_warn(sum(theta)/2)

    # class generation
    com_size = com_size_gen(k-nb_active,phi_c)
    if nb_active:
        active_com_size = np.array(size_active) / n
        for mod in active_com_size:
            com_size[random.randrange(len(com_size)-1)] -= mod
        com_size = np.concatenate((active_com_size, com_size))
